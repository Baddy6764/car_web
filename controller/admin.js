...
exports.Registercars =  async(req, res) => {
  try {
   ...
   const imageFileUploaded = await cloudinary.uploader.upload(imageFile,{resource_type:"image"})
   ...
   if(imageFileTwo){
    const resultTwo = await cloudinary.uploader.upload(imageFileTwo,{resource_type:"image"});
